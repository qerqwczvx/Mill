...
def group_statistics(call):
    ...
    items_per_page = 5  # Ограничиваем до 5 групп на страницу
        ...
        for owner_id, take_date, shutdown_date, status in numbers_data:
            if take_date not in ("0", "1") and shutdown_date not in ("0", "1"):
                ...
                if status == 'отстоял 1/2' and holds_count >= 1:
                    earnings = min(PRICE_ADM, 6.0)
                elif status in ['отстоял 2/2', 'отстоял 2/2+ холд']:
                    earnings = min(min(holds_count, MAX_HOLDS) * PRICE_ADM, 12.0)
                else:
                    earnings = 0.0
    ...
    markup = types.InlineKeyboardMarkup()
    for group_id, group_name in page_groups:
        markup.add(types.InlineKeyboardButton(f"📊 {group_name[:20]}", callback_data=f"group_stats_{group_id}_1"))  # Ограничиваем название до 20 символов
        ...
        if not page_numbers:
            ...
            for number, take_date, shutdown_date in page_numbers:
                try:
                    ...
                    minutes = (shutdown_time - take_time).total_seconds() / 60
                    rounded_minutes = (minutes // 30) * 30
                    time_str = f"{int(rounded_minutes)}"
