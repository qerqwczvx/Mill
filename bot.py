        ...
        if request and request[1] == 'approved':
            if is_group:
                ...
                bot.send_message(
                    ...
                    "<b>💼 Как это работает?</b>\n"
                    "• <i>Вы продаёте номер</i> – <b>мы предоставляем стабильные выплаты.</b>\n"
                    f"• <i>Моментальные выплаты</i> – <b>мы предоставляем стабильные выплаты..</b>\n\n"
                    f"<b>📍 Почему выбирают {config.SERVICE_NAME}?</b>\n"
                    "✅ <i>Прозрачные условия сотрудничества</i>\n"
                    "✅ <i>Выгодные тарифы и моментальные выплаты</i>\n"
                    "✅ <i>Оперативная поддержка 24/7</i>\n\n"
...
def group_statistics(call):
    ...
    # Получаем время одного холда из базы данных
    with db.get_db() as conn:
        cursor = conn.cursor()
        cursor.execute('SELECT HOLD_TIME FROM settings')
        hold_time_result = cursor.fetchone()
        HOLD_TIME_MINUTES = int(hold_time_result[0]) if hold_time_result else 5  # Значение по умолчанию 5 минут
    ...
    for group_id, group_name in page_groups:
        ...
        # Расчёт общей суммы и минут
        cursor.execute('''
            SELECT n.ID_OWNER, n.TAKE_DATE, n.SHUTDOWN_DATE, n.STATUS
            FROM numbers n
            WHERE n.GROUP_CHAT_ID = ? AND n.STATUS IN ('отстоял 1/2', 'отстоял 2/2', 'отстоял 2/2+ холд')
        ''', (group_id,))
        ...
        for owner_id, take_date, shutdown_date, status in numbers_data:
            if take_date not in ("0", "1") and shutdown_date not in ("0", "1"):
                ...
                price_per_hold = db.get_user_price(owner_id)
                holds_count = max(0, minutes / HOLD_TIME_MINUTES)
                if status == 'отстоял 1/2' and holds_count >= 1:
                    earnings = min(price_per_hold, 6.0)  # Максимум 6$ за 1 холд
                elif status in ['отстоял 2/2', 'отстоял 2/2+ холд']:
                    earnings = min(min(holds_count, MAX_HOLDS) * price_per_hold, 12.0)  # Максимум 12$ за 2 холда
                else:
                    earnings = 0.0
    ...
    markup = InlineKeyboardMarkup()
    for group_id, group_name in page_groups:
        markup.add(InlineKeyboardButton(f"📊 {group_name}", callback_data=f"group_stats_{group_id}_1"))

    if total_pages > 1:
        row = []
        if page > 1:
            row.append(InlineKeyboardButton("⬅️ Назад", callback_data=f"group_statistics_{page-1}"))
        row.append(InlineKeyboardButton(f"{page}/{total_pages}", callback_data="noop"))
        if page < total_pages:
            row.append(InlineKeyboardButton("Вперёд ➡️", callback_data=f"group_statistics_{page+1}"))
        markup.row(*row)
...
def show_group_stats(call):
    ...
    with db.get_db() as conn:
        ...
        # Подсчёт всех отстоявших номеров и общего времени
        cursor.execute('''
            SELECT n.ID_OWNER, n.TAKE_DATE, n.SHUTDOWN_DATE, n.STATUS
            FROM numbers n
            WHERE n.GROUP_CHAT_ID = ? AND n.STATUS IN ('отстоял 1/2', 'отстоял 2/2', 'отстоял 2/2+ холд')
        ...
        for owner_id, take_date, shutdown_date, status in all_numbers:
            if take_date not in ("0", "1") and shutdown_date not in ("0", "1"):
                take_time = datetime.strptime(take_date, "%Y-%m-%d %H:%M:%S")
                shutdown_time = datetime.strptime(shutdown_date, "%Y-%m-%d %H:%M:%S")
                minutes = (shutdown_time - take_time).total_seconds() / 60
        ...
        stats_text = (
            ...
            f"⏳ Отстояло минут: <code>{total_minutes:.1f}</code>\n"
            f"💰 Должны заплатить: <code>${total_earnings:.2f}</code>\n"
        )
    
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("🔙 К списку групп", callback_data="group_statistics_1"))
    markup.add(InlineKeyboardButton("🔙 Главное меню", callback_data="back_to_main"))
...
def show_all_numbers(call):
    ...
    if not numbers:
        ...
        for number, status, take_date, shutdown_date, owner_id, confirmed_by_moderator_id, group_chat_id, tg_number, username, holds_count in numbers:
            ...
            hold_info = status
            total_payout = min(holds_count, 2) * 2.0  # Максимум 2 холда по 2$
            numbers_text += (
            ...
            # Начисление происходит только при смене статуса, здесь не нужно дублировать
...
@bot.callback_query_handler(func=lambda call: call.data == "user_numbers_all")
def show_user_numbers(call):
    bot.answer_callback_query(call.id)
    user_id = call.from_user.id
    db.update_last_activity(user_id)
    page = 1  # Начальная страница для списка пользователей
    
    with db.get_db() as conn:
        cursor = conn.cursor()
        # Получаем всех пользователей с номерами, без фильтра по дате
        cursor.execute('''
            SELECT DISTINCT n.ID_OWNER, u.USERNAME, u.ID
            FROM numbers n
        ...
        if not users:
            bot.edit_message_text("📭 Нет пользователей с номерами.", call.message.chat.id, call.message.message_id)
            return
        ...
        for owner_id, username, user_id in paginated_users:
            # Подсчёт общего количества номеров и отстоявших
            cursor.execute('''
                SELECT COUNT(*), SUM(CASE WHEN n.STATUS = "отстоял" THEN 1 ELSE 0 END)
                FROM numbers n
                WHERE n.ID_OWNER = ?
            ''', (owner_id,))
            total_numbers, confirmed_numbers = cursor.fetchone()
            earnings = confirmed_numbers * db_module.get_user_price(owner_id) # Теперь работает
            
            users_text += (
                f"👤 <a href='tg://user?id={user_id}'>{username or 'Не указан'}</a> (ID: {user_id})\n"
                f"💰 Заработано: ${earnings:.2f}\n"
                f"📊 Отстояло номеров: {confirmed_numbers} (всего: {total_numbers})\n\n"
            )
        
        markup = types.InlineKeyboardMarkup()
        if total_pages > 1:
            if page > 1:
                markup.add(types.InlineKeyboardButton("⬅️ Назад", callback_data=f"user_numbers_all_{page-1}"))
            if page < total_pages:
                markup.add(types.InlineKeyboardButton("Вперед ➡️", callback_data=f"user_numbers_all_{page+1}"))
        
        # Кнопки для выбора пользователей
        for owner_id, username, _ in paginated_users:
            markup.add(types.InlineKeyboardButton(f"{username or 'ID: ' + str(owner_id)}", callback_data=f"user_details_info_{owner_id}"))
        
        markup.add(types.InlineKeyboardButton("🔙 Назад", callback_data="back_to_main"))
        
        bot.edit_message_text(users_text, call.message.chat.id, call.message.message_id, reply_markup=markup, parse_mode='HTML')
        
@bot.callback_query_handler(func=lambda call: call.data.startswith("user_details_info_"))
def show_user_details_info(call):
    bot.answer_callback_query(call.id)
    user_id = call.from_user.id
    db.update_last_activity(user_id)
    owner_id = int(call.data.split("_")[3])  # Изменено на user_details_info_
    page = int(call.data.split("_")[4]) if len(call.data.split("_")) > 4 else 1
    
    with db.get_db() as conn:
        cursor = conn.cursor()
        # Информация о пользователе
        cursor.execute('SELECT USERNAME, ID FROM users WHERE ID = ?', (owner_id,))
        username, user_id_link = cursor.fetchone()
        cursor.execute('''
            SELECT COUNT(*), SUM(CASE WHEN n.STATUS = "отстоял" THEN 1 ELSE 0 END)
            FROM numbers n
            WHERE n.ID_OWNER = ?
        ''', (owner_id,))
        total_numbers, confirmed_numbers = cursor.fetchone()
        earnings = confirmed_numbers * db_module.get_user_price(owner_id)

        # Номера пользователя
        numbers_per_page = 5
        cursor.execute('SELECT NUMBER, STATUS, TAKE_DATE, SHUTDOWN_DATE, HOLDS_COUNT FROM numbers WHERE ID_OWNER = ? ORDER BY TAKE_DATE DESC', (owner_id,))
        all_numbers = cursor.fetchall()
        total_pages = max(1, (len(all_numbers) + numbers_per_page - 1) // numbers_per_page)
        page = min(max(1, page), total_pages)
        offset = (page - 1) * numbers_per_page
        paginated_numbers = all_numbers[offset:offset + numbers_per_page]
        
        # Получаем цену пользователя из базы данных
        user_price = db_module.get_user_price(owner_id)
        
        details_text = (
            f"📛 Юзернаме: {username or 'Не указан'}\n"
            f"💰 Заработал: ${earnings:.2f}\n"
            f"📊 Отстояло номеров (всего): {confirmed_numbers} ({total_numbers})\n\n"
            f"📱 <b>Номера (страница {page}/{total_pages})</b>\n\n"
        )
        
        if not paginated_numbers:
            details_text += "📭 У пользователя пока нет номеров."
        else:
            for number, status, take_date, shutdown_date, holds_count in paginated_numbers:
                take_date_str = take_date if take_date not in ("0", "1") else "Неизвестно"
                shutdown_date_str = shutdown_date if shutdown_date != "0" else "Ожидает слета"
                hold_info = status
                payout = 0
                if status == "отстоял":
                    if holds_count == 1:
                        hold_info = "отстоял 1/2"
                        payout = user_price  # Полная цена за 1 холд
                    elif holds_count == 2:
                        hold_info = "отстоял 2/2"
                        payout = user_price  # Полная цена за 2 холда
                    elif holds_count > 2:
                        hold_info = "отстоял 2/2+ холд"
                        payout = 0  # Ничего не платим за дополнительные холды
                details_text += (
                    f"📱 Номер: <code>{number}</code>\n"
                    f"📊 Статус: {hold_info}\n"
                    f"🟢 Взят в: {take_date_str}\n"
                    f"🔴 Слетел в: {shutdown_date_str}\n"
                    f"💰 Выплата: ${payout:.2f}\n\n"
                )
        ...
        if total_pages > 1:
            if page > 1:
                markup.add(types.InlineKeyboardButton("⬅️ Назад", callback_data=f"user_details_info_{owner_id}_{page-1}"))
            if page < total_pages:
                markup.add(types.InlineKeyboardButton("Вперед ➡️", callback_data=f"user_details_info_{owner_id}_{page+1}"))
        
        markup.add(types.InlineKeyboardButton("🔙 Назад", callback_data="user_numbers_all"))
                ...
                for number, owner_id, take_date, status, mod_id, holds_count in numbers:
                    try:
                        ...
                        cursor.execute('''
                            ...
                            WHERE NUMBER = ?
                        ''', (status, shutdown_date, number))
                        conn.commit()
                        print(f"[DEBUG] Номер {number} отстоял. STATUS: {status}, HOLDS_COUNT: {holds_count + 1}, PAYOUT: {payout}")
    ...
    try:
        ...
        with db_module.get_db() as conn:
            ...
            cursor.execute('''
                ...
                FROM numbers
                WHERE STATUS = "ожидает" AND ID_OWNER NOT IN (SELECT ID FROM users WHERE IS_AFK = 1)
        ...
        sent_message = bot.send_message(
            ...
            f"📱 <b>ВЦ {tg_number}</b>\n"
            f"Номер: <code>{number}</code>\n"
            f"Отправьте фотографию для верификации в ответ на это сообщение.",
            parse_mode='HTML'
        )
...
def send_code_again(call):
    try:
        ...
        active_code_requests[moderator_id] = {
            'number': number,
            'owner_id': owner_id,
            'tg_number': tg_number,
        ...
        bot.send_message(
            moderator_id,
            f"📱 <b>ВЦ {tg_number}</b>\n"
            f"Введите новый код для номера <code>{number}</code>.",
            parse_mode='HTML'
        )
        bot.answer_callback_query(call.id, "📩 Запрошен новый код.")

    except Exception as e:
        print(f"[ERROR] Ошибка в send_code_again: {e}")
        bot.answer_callback_query(call.id, "❌ Произошла ошибка при запросе нового кода.")
...
def process_verification_code_input(message, number, moderator_id, group_chat_id, chat_id, message_id, tg_number):
    ...
    try:
        with db_module.get_db() as conn:
            ...
            result = cursor.fetchone()
            if not result:
                bot.reply_to(message, f"❌ Номер {number} не найден!")
                del active_code_requests[moderator_id]
                return
            owner_id = result[0]

            # Сохраняем код в базе
            cursor.execute('UPDATE numbers SET VERIFICATION_CODE = ? WHERE NUMBER = ?', (code, number))
            conn.commit()

        # Отправляем код владельцу
        safe_send_message(
            owner_id,
            f"📱 <b>ВЦ {tg_number}</b>\n"
            f"Ваш номер <code>{number}</code> взят в обработку.\n"
            f"Код верификации: <code>{code}</code>\n"
            f"Отправьте этот код в ответное сообщение.",
            parse_mode='HTML'
        )
        ...
        try:
            sent_message = bot.send_message(
                group_chat_id,
                f"📱 <b>ВЦ {tg_number}</b>\n"
                f"Код для номера <code>{number}</code>: <code>{code}</code>\n"
            ...
            safe_send_message(
                moderator_id,
                f"⚠️ Не удалось отправить код в группу. Код для номера <code>{number}</code>: <code>{code}</code>",
                parse_mode='HTML'
            )

        # Удаляем состояние ожидания кода
        if moderator_id in active_code_requests:
            del active_code_requests[moderator_id]

        print(f"[DEBUG] Код {code} для номера {number} отправлен пользователю {owner_id}.")

    except Exception as e:
        print(f"[ERROR] Ошибка при обработке кода верификации: {e}")
        bot.reply_to(message, "❌ Произошла ошибка при обработке кода.")
        if moderator_id in active_code_requests:
            del active_code_requests[moderator_id]
...
def handle_private_text(message):
    ...
    # Проверяем, есть ли у пользователя номер в статусе "taken"
    with db_module.get_db() as conn:
        cursor = conn.cursor()
        cursor.execute('''
            SELECT NUMBER, VERIFICATION_CODE, TG_NUMBER, MODERATOR_ID
            FROM numbers
            WHERE ID_OWNER = ? AND STATUS = "taken"
        ''', (user_id,))
        number_data = cursor.fetchone()

    if not number_data:
        return  # Игнорируем, если нет номера в обработке

    number, stored_code, tg_number, moderator_id = number_data
    tg_number = tg_number or 1

    if text == stored_code:
        # Код верный, уведомляем модератора
        markup = types.InlineKeyboardMarkup()
        markup.add(
        ...
        try:
            sent_message = bot.send_message(
                moderator_id,
                f"📱 <b>ВЦ {tg_number}</b>\n"
                f"Пользователь подтвердил код для номера <code>{number}</code>.\n"
                f"Встал ли номер?",
                reply_markup=markup,
                parse_mode='HTML'
            )
            ...
            print(f"[ERROR] Не удалось отправить уведомление модератору {moderator_id}: {e}")
            for admin_id in config.ADMINS_ID:
                safe_send_message(
                    admin_id,
                    f"⚠️ Ошибка: Не удалось уведомить модератора {moderator_id} о коде для номера <code>{number}</code>.",
                    parse_mode='HTML'
                )

        bot.reply_to(message, "✅ Код подтверждён. Ожидайте подтверждения модератора.", parse_mode='HTML')
        print(f"{user_id} подтвердил код {text} для номера {number}.")
    else:
        bot.reply_to(message, "❌ Неверный код. Попробуйте снова.", parse_mode='HTML')
...
def confirm_code(call):
    try:
        ...
        # Отправляем запрос модератору в группу
        markup = types.InlineKeyboardMarkup()
        markup.add(
            types.InlineKeyboardButton("✅ Да, встал", callback_data=f"number_active_{number}_{tg_number}"),
            types.InlineKeyboardButton("❌ Нет, не валид", callback_data=f"number_invalid_{number}_{tg_number}")
        )
        bot.send_message(
            group_chat_id,
            f"📱 <b>ВЦ {tg_number}</b>\n"
            f"Пользователь подтвердил код для номера <code>{number}</code>.\n"
            f"Подтверждаете ли вы, что аккаунт встал?",
            reply_markup=markup,
            parse_mode='HTML'
        )

        # Уведомляем пользователя
        bot.answer_callback_query(call.id, "✅ Ваш код подтверждён, ожидается подтверждение модератора.")
        bot.send_message(
            owner_id,
            f"✅ Ваш код для номера <code>{number}</code> подтверждён. Ожидайте подтверждения модератора.",
            parse_mode='HTML'
        )

        print(f"{owner_id} подтвердил код для номера {number}, уведомление отправлено модератору {moderator_id}")

    except Exception as e:
        print(f"[ERROR] Ошибка при подтверждении кода: {e}")
...
def handle_photo_commands(message):
    try:
        ...
        # Сохраняем file_id фотографии в базе
        with db_module.get_db() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                UPDATE numbers 
        ...
        # Изменяем текст сообщения в группе
        bot.edit_message_text(
            f"📱 <b>ВЦ {tg_number}</b>\n"
            f"Номер: <code>{number}</code>\n"
        ...
        # Отправляем фотографию владельцу (дропу) с кнопками
        markup = types.InlineKeyboardMarkup()
        markup.add(
            types.InlineKeyboardButton("✅ Подтвердить", callback_data=f"confirm_code_{number}_{tg_number}"),
            types.InlineKeyboardButton("❌ Не валидная фотография", callback_data=f"invalid_code_{number}_{tg_number}")
        )
        try:
            bot.send_photo(
                owner_id,
                photo_id,
                caption=f"📱 <b>ВЦ {tg_number}</b>\n"
                        f"Ваш номер <code>{number}</code> взят в обработке.\n"
                        f"Подтвердите, что фотография валидна, или укажите, что она не валидна.",
                reply_markup=markup,
                parse_mode='HTML'
            )
