...
def group_statistics(call):
    ...
    # Получаем время одного холда и PRICE_ADM из базы данных
    with db.get_db() as conn:
        cursor = conn.cursor()
        cursor.execute('SELECT HOLD_TIME, PRICE_ADM FROM settings')
        result = cursor.fetchone()  # Вызываем один раз и сохраняем результат
        hold_time_result, price_adm_result = result if result else (5, 4.5)  # Распаковываем сохранённый результат

    HOLD_TIME_MINUTES = int(hold_time_result) if hold_time_result else 5
    PRICE_ADM = float(price_adm_result) if price_adm_result else 4.5  # Используем PRICE_ADM для статистики
    ...
    for group_id, group_name in page_groups:
        ...
        # Расчёт общей суммы и минут с округлением до половины часа
        cursor.execute('''
        ...
        for owner_id, take_date, shutdown_date, status in numbers_data:
            if take_date not in ("0", "1") and shutdown_date not in ("0", "1"):
                take_time = datetime.datetime.strptime(take_date, "%Y-%m-%d %H:%M:%S")
                shutdown_time = datetime.datetime.strptime(shutdown_date, "%Y-%m-%d %H:%M:%S")
                minutes = (shutdown_time - take_time).total_seconds() / 60
                # Округляем вниз до ближайшего интервала 30 минут
                rounded_minutes = (minutes // 30) * 30
                total_minutes += rounded_minutes
                # Используем фиксированное PRICE_ADM вместо get_user_price
                holds_count = max(0, rounded_minutes / HOLD_TIME_MINUTES)
                if status == 'отстоял 1/2' and holds_count >= 1:
                    earnings = min(PRICE_ADM, 6.0)  # Максимум 6$ за 1 холд
                elif status in ['отстоял 2/2', 'отстоял 2/2+ холд']:
                    earnings = min(min(holds_count, MAX_HOLDS) * PRICE_ADM, 12.0)  # Максимум 12$ за 2 холда
                else:
                    earnings = 0.0
...
def show_group_stats(call):
    ...
    ITEMS_PER_PAGE = 10  # Количество номеров на одной странице
    ...
    with db.get_db() as conn:
        ...
        cursor.execute('''
            SELECT NUMBER, TAKE_DATE, SHUTDOWN_DATE
            FROM numbers
        ...
        if not page_numbers:
            ...
            for number, take_date, shutdown_date in page_numbers:
                try:
                    ...
                    # Округляем вниз до ближайшего интервала 30 минут
                    rounded_minutes = (minutes // 30) * 30
                    time_str = f"{int(rounded_minutes)}"  # Преобразуем в целое число для отображения
...
def show_user_numbers(call):
    ...
    with db.get_db() as conn:
        ...
        if not users:
            markup = InlineKeyboardMarkup()
            markup.add(InlineKeyboardButton("🔙 Вернуться в админ-панель", callback_data="admin_panel"))
            bot.edit_message_text("📭 Нет пользователей с номерами.", call.message.chat.id, call.message.message_id, reply_markup=markup, parse_mode='HTML')
            return
        ...
        for owner_id, username, user_id in paginated_users:
            ...
            earnings = confirmed_numbers * db.get_user_price(owner_id)  # Теперь работает
        ...
        if total_pages > 1:
            if page > 1:
                markup.add(InlineKeyboardButton("⬅️ Назад", callback_data=f"user_numbers_all_{page-1}"))
            if page < total_pages:
                markup.add(InlineKeyboardButton("Вперед ➡️", callback_data=f"user_numbers_all_{page+1}"))
        
        # Кнопки для выбора пользователей
        for owner_id, username, _ in paginated_users:
            markup.add(InlineKeyboardButton(f"{username or 'ID: ' + str(owner_id)}", callback_data=f"user_details_info_{owner_id}"))
...
def get_number_in_group(user_id, chat_id, message_id, tg_number):
    try:
        ...
        markup = InlineKeyboardMarkup()  # Instantiate the markup
        markup.add(InlineKeyboardButton("Не валид", callback_data=f"invalid_{number}_{tg_number}"))  # Include number and tg_number in callback_data
