...
def show_user_numbers(call):
    ...
    try:
        ...
        with db.get_db() as conn:
            cursor = conn.cursor()
            # Получаем всех пользователей с номерами
            cursor.execute('''
...
def show_user_details(call):
    try:
        ...
        with db.get_db() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT USERNAME, ID, BALANCE, REG_DATE, IS_AFK FROM users WHERE ID = ?', (target_user_id,))
            user = cursor.fetchone()
            if not user:
                bot.answer_callback_query(call.id, "❌ Пользователь не найден!")
                return

            username, user_id, balance, reg_date, is_afk = user
            cursor.execute('''
...
def process_error(call):
    try:
        ...
        user_id = call.from_user.id
        chat_id = call.message.chat.id  # Исправлено с chat.chat_id на chat.id
