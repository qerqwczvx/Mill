...
def group_statistics(call):
    ...
    cursor.execute('SELECT value FROM settings WHERE key = ?', ('hold_time_minutes',))
    hold_time_result = cursor.fetchone()
    HOLD_TIME_MINUTES = int(hold_time_result[0]) if hold_time_result else 6  # Значение по умолчанию 6 минут
...
def show_group_stats(call):
    ...
    with db.get_db() as conn:
        ...
        cursor.execute('''
            SELECT n.ID_OWNER, n.NUMBER, n.TAKE_DATE, n.SHUTDOWN_DATE, n.STATUS
            FROM numbers n
            WHERE n.GROUP_CHAT_ID = ? AND n.STATUS IN ('отстоял 1/2', 'отстоял 2/2', 'отстоял 2/2+ холд')
    ...
    if not recent_numbers:
        ...
        for owner_id, number, take_date, shutdown_date, status in recent_numbers:
            ...
            stats_text += (
                ...
                f"⏳ Минуты: <code>{minutes:.1f}</code>\n"
                f"💰 Оплата: <code>${earnings:.2f}</code>\n"
